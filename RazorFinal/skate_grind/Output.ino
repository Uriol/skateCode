...
// boolean for when it is grinding
...
//  Serial.print(cz); Serial.println();
 ...
 // Timer 
  ...
  if (start == true) {
    if (jumping == false && grinding == false) {
      on_the_ground();
    }
    
    if ( jumping == false && grinding == true ) {
      skate_grinding();
    }
...
void in_the_air(){ // while jumping calculates : time in the air, pos X,Y. First jump, initial yaw on jumping
  ...
  if (cz <= 0.35 && cz >= -0.35) {
    ...
    if ( jumping == false ) { // Serial.println(" First JUMP "); 
    if ( firstJump == false ) { initialYaw_onJumping = yaw; }
    if ( firstJump == true ) { secondJump = true; Serial.println("Second jump is true ");  }
    initialJumpSpeed(); calculateJump(); 
    Serial.print( "initialYaw_onJumping "); Serial.println(initialYaw_onJumping); 
    
    firstJump = true;
    Serial.println("first jump is true ");
  }
  
    // if it was grinfing before the jump means that jumps out the grind
    if (grinding == true ) { grinding = false; }
...
void skate_grinding(){
  Serial.println("skateboard grinding");
