...
float totalSpeed = 3;
String csvFile = "heelflip180.csv";
...
float firstJumpSpeed =-0.2; // for ollie180
...
void setup() {
  ...
  name = loadImage("heelflip180.png");
  
  video = new Movie(this, "heel180_960.mov");
  video.loop();
...
airtimeImage = loadImage("airtime.png");
...
void gui() {
  ...
  // jump distance
   pushMatrix();
   translate(0,90,0); 
  image(jumpDistance, width-360, 0, 118,35); // 89
   textFont(myFont,15);
   fill(184, 228, 0);
  ...
  // airtime
   pushMatrix();
   translate(0,90*2,0); 
  image(airtimeImage, width-360, 0, 131,35); // 89
   textFont(myFont,15);
   fill(184, 228, 0);
  ...
  // backside
   pushMatrix();
   translate(0,90*4,0); 
  image(backside, width-360, 0, 82,35); // 89
   textFont(myFont,15);
   fill(184, 228, 0);
  text("BACKSIDE ROTATION ON AIR", width-360, 60); //+16
  stroke(50);
  //line(width-360, 75, width-40, 75);
  popMatrix();
